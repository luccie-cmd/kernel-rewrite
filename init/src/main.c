...
static DynArrayHeader* stringToNumber(const char* str) {
...
static void mountPartitions(const char* data) {
    ...
    while (line) {
        ...
        PUSH_DIAG(GCC diagnostic ignored "-Wanalyzer-out-of-bounds");
        dynarray(uint8_t) numPartID = (uint8_t*)stringToNumber(partUUID) + sizeof(DynArrayHeader);
        if (!numPartID) {
