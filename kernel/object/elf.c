...
static bool verifyEhdr(Elf64_Ehdr* ehdr) {
    if (memcmp(ehdr->e_ident, ELFMAG, SELFMAG) != 0) {
        debug("Invalid ELF magic (Got %x, expected %x)\n", *(uint32_t*)ehdr->e_ident,
              *(uint32_t*)(uint8_t*)ELFMAG);
        return false;
    }
    return true;
}
...
ElfFile* loadElfObject(uint64_t fileHandle, size_t PHDRAddend, const char* depPrefix) {
    ...
    if (!verifyEhdr(hdr)) {
        debug("Invalid header (depPrefix: %s)\n", depPrefix);
        free(hdr);
        return NULL;
    }
    ...
    for (Elf64_Half i = 0; i < phnum; i++) {
        ...
        if (phdr->p_type == PT_LOAD) {
            ...
            char*    str    = malloc(phdr->p_filesz);
            uint64_t offset = vfsGetOffset(fileHandle);
            vfsSeek(fileHandle, phdr->p_offset);
            vfsRead(fileHandle, str, phdr->p_filesz);
            vfsSeek(fileHandle, offset);
            uint64_t interpFd = vfsOpen(str, OPEN_FLAG_READ);
            file->interpreter = loadElfObject(interpFd, 0, "/lib");
            vfsClose(interpFd);
            todo(true, "Handle interpreter `%s`\n", str);
            free(str);
        }
    }
    return file;
