...
// void  __attribute__((no_sanitize_address)) __ubsan_handle_implicit_conversion(void* _data, void*
// lhs, void* rhs) {}
...
void UBSAN_NOSAN __ubsan_handle_nonnull_return_v1(struct nonnull_return_data_v1* data,
...
void UBSAN_NOSAN __ubsan_handle_float_cast_overflow(struct overflow_data* data, void* from) {
    char old_val_str[VALUE_LENGTH];
    if (suppress_report(&data->location)) return;
    ubsan_prologue(&data->location, "negation-overflow");
...
void UBSAN_NOSAN __ubsan_handle_function_type_mismatch(struct function_type_mismatch_data* data,
                                                       void*                               from) {
    char old_val_str[VALUE_LENGTH];
    if (suppress_report(&data->location)) return;
    ubsan_prologue(&data->location, "negation-overflow");
